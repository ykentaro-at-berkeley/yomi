...
type player_id = int [@@deriving typerep]
type deal_type = No_basi | No_basanbon
type yaku_join_type = Before_megati | After_megati
type oni_type = Leave_rest | Take_rest
type megati =
  | Megati_none of util (* origin *)
  | Megati_with_origin of util * util (* origin, outcome *)
  | Megati_without_origin of util
  | Megati_fun of (util list -> util list)
...
module Make (G : GAME) = struct
  ...
  type game' =
    { current : player_id;
      ba : card list;
      yama : card list;
      players : player list } [@@deriving typerep]

  type game_phase =
    | Play_phase
    | Awase1_phase of card (* lastly played card *)  
    | Awase2_phase of card (* lastly drawn card *)
    | Winning_phase (* won with a yaku *)
    | Thru_phase (* run out of tehuda *) [@@deriving typerep]

  type game =
    { phase : game_phase;
      data : game' } [@@deriving typerep]

  let player_of_game' ?player { current; players } =
    match player with
    | None ->  List.nth players current
    | Some p -> List.nth players p
  let player_of_game ?player g = player_of_game' ?player:player g.data

  let update_current_player' ({ current = c } as g') p =
    { g' with players = List.set_at_idx c p g'.players }
  ...
  type move =
    ...
    | Awase1 : card ->  move
    | Awase1_nop : move
    | Awase1_basanbon : card * card * card -> move
    | Awase2 : card * card -> move (* The card drawn comes first *)
    | Awase2_basanbon : card * card * card * card -> move
    | Awase2_nop : card -> move
  ...
  let payoff_thru : player_id -> game -> util
    ...
    match G.megati with
    | Megati_none origin ->
       util_of_tori_trivial (player_of_game ~player:p g).tori + payoff_winning p g
       - origin
    | _ ->
       let us =
         match G.yaku_join_type with
         | Before_megati ->
            let f p = util_of_tori_trivial (player_of_game ~player:p g).tori
                      + payoff_winning p g in
            List.init n_players f
         | After_megati ->
            let f p = util_of_tori_trivial (player_of_game ~player:p g).tori in
            List.init n_players f in
       let u = 
         match G.megati with
         | Megati_none _ -> assert false
         | Megati_with_origin (origin, util) ->
            let f (idx, me, cnt_above, cnt_below) u =
              let me = me || (p = idx && u > origin) in
              let cnt_above = cnt_above + (if u > origin then 1 else 0) in
              let cnt_below = cnt_below + (if u < origin then 1 else 0) in
              (idx + 1, me, cnt_above, cnt_below) in
            let _, me, cnt_above, cnt_below =
              List.fold_left f (0, false, 0, 0) us in
            if me then cnt_below * util
            else if List.nth us p = origin then 0
            else ~- (cnt_above * util)
         | Megati_without_origin util ->
            (* I don't know what to do in case of a tie...
             * Let's say that the eldest player wins *)
            let f (idx, max, i_max) u =
              if u > max then (idx + 1, u, idx)
              else (idx + 1, max, i_max) in
            let _, _, i_max = List.fold_left f (0, min_int, -1) us in
            if p = i_max then (G.n_players - 1) * util
            else -util (* is 0 really OK? *)
         | Megati_fun f -> List.nth (f us) p in
       match G.yaku_join_type with
       | After_megati -> payoff_winning p g + u
       | Before_megati -> u

  let payoff p g =
    match g with
    | { phase = Winning_phase } as g -> Some (payoff_winning p g)
    | { phase = Thru_phase } as g -> Some (payoff_thru p g)
    | _ -> None
  ...
  let pass' ({ current } as g) =
    if current >= n_players - 1 then { g with current = 0 }
  ...
  let apply_awase2 { phase = Awase2_phase c; data = data } m =
    ...
    let k data =
      ...
      | Abort_when_yaku ->
         ...
         | _ ->  { phase = Winning_phase; data = data } in
    match m with
  ...
  let apply : game -> move -> game
    ...
    match g with
    | { phase = Awase1_phase _ } as g ->  (apply_awase1_ g m)
    | { phase = Awase2_phase _} as g -> apply_awase2 g m
    | { phase = Play_phase } as g ->  (apply_play_ g m)
    | { phase = Winning_phase } ->
  ...
  let random :game -> game =
    fun g ->
    let p = player_of_game g in
    let cs_tori =
      let f cs_tori player =
        player.tori.data @ cs_tori in
      List.fold_left f [] g.data.players in
    let additional =
      match g with
      ...
      | _ -> [] in
    let visible =
      additional @ p.hand @ g.data.ba @ cs_tori in
    let cs = diff_card hana_karuta visible in
    let cs = Random.shuffle_list cs in
    let players, cs =
      let f player (players, cs) =
        if player == p then (* physical id *)
          (p :: players, cs)
        else
          let hand, cs = take_drop_wo_si (List.length player.hand) cs in
          ({ player with hand } :: players, cs) in
      List.fold_right f g.data.players ([], cs) in
    (* Printf.fprintf stderr "cs: %d; g.data.yama: %d\n"
     *   (List.length cs) (List.length g.data.yama); *)
    assert (List.length cs = List.length g.data.yama);
    List.iter2 (fun p p' -> assert (List.length p.hand = List.length p'.hand)) players g.data.players;
    { g with data = { g.data with yama = cs; players }}

  let init () =
    let cs = Random.shuffle_list hana_karuta in
    let ba, cs = take_drop_for_ba G.n_ba cs in
    let players, cs = 
      let rec loop i players cs =
        if i >= n_players then players, cs
        else
  ...
  module MCUCB1 = struct
    ...
    let rec simple_playout : player_id -> game -> move -> float =
      fun p g m ->
      (* begin *)
      (*   match m with *)
      (*   | Play (m, r) -> *)
    ...
    let add_one_playout :
    game -> move list -> stat array -> unit =
      fun game ms ts ->
